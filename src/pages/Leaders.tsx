...
const getAvatarGradient = (name: string) => {
  const colors = ['bg-gradient-to-br from-blue-100 to-blue-200 text-blue-700', 'bg-gradient-to-br from-green-100 to-green-200 text-green-700', 'bg-gradient-to-br from-purple-100 to-purple-200 text-purple-700', 'bg-gradient-to-br from-amber-100 to-amber-200 text-amber-700', 'bg-gradient-to-br from-rose-100 to-rose-200 text-rose-700', 'bg-gradient-to-br from-cyan-100 to-cyan-200 text-cyan-700'];
...
const getHierarchyBadge = (leader: OfficeLeader) => {
  ...
  if (leader.hierarchy_level && leader.hierarchy_level > 1) {
    ...
    return {
      label: `Nível ${level}`,
      className: colors[level] || "bg-muted text-muted-foreground"
    };
  }
  return null;
};
const getBirthdayBadge = (leader: OfficeLeader) => {
  ...
  if (days === 0) {
    return {
      label: "🎂 Hoje!",
      className: "bg-pink-100 text-pink-700 border-pink-300 animate-pulse"
    };
  } else if (days === 1) {
    return {
      label: "🎂 Amanhã!",
...
const Leaders = () => {
  ...
  useEffect(() => {
    const channel = supabase.channel('leaders-realtime').on('postgres_changes', {
      event: 'UPDATE',
      schema: 'public',
      table: 'lideres'
    }, payload => {
  ...
  const {
    data: subordinatesCounts
  } = useLeadersSubordinatesCounts(leaderIds);

  // Buscar contagem de líderes verificados para notificações em massa
  const {
    data: verifiedLeadersCount
  } = useQuery({
    ...
    queryFn: async () => {
      const {
        count,
        error
      } = await supabase.from("lideres").select("*", {
        count: "exact",
        head: true
  ...
  const fetchAllVerifiedLeaderIds = useCallback(async (): Promise<string[]> => {
    const {
      data,
      error
    } = await supabase.from("lideres").select("id").eq("is_active", true).eq("is_verified", true);
    if (error) throw error;
  ...
  return <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <SendPassNotificationDialog allVerifiedCount={verifiedLeadersCount || 0} onSendToAll={fetchAllVerifiedLeaderIds}>
      ...
      <div className="bg-card border rounded-lg p-4 mb-6">
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[200px] max-w-xs">
            ...
            <Input placeholder="Buscar líder..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9" />
          </div>
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <Button variant="outline" size="icon" onClick={handleManualRefresh} disabled={isRefreshing} title="Atualizar lista">
      ...
      <div className="space-y-3">
        {isLoading ? <Card>
          ...
          </Card> : leaders.map(leader => <Card key={leader.id} className={getLeaderCardColorClass(leader.pontuacao_total, leaderLevels)}>
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className="relative flex-shrink-0">
                      ...
                      {leader.is_coordinator && <div className="absolute -top-1 -right-1 w-5 h-5 bg-amber-400 rounded-full flex items-center justify-center shadow-sm">
                    ...
                    return <Badge variant="outline" className={`text-xs ${hierarchyBadge.className}`}>
                    ...
                    return <Badge variant="outline" className={`text-xs ${birthdayBadge.className}`}>
                              {birthdayBadge.label}
                            </Badge>;
                  })()}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        ...
                        <Badge variant={leader.is_active ? "default" : "secondary"} className={leader.is_active ? "bg-green-500/10 text-green-600 border-0" : ""}>
                          {leader.is_active ? '✓ Ativo' : 'Inativo'}
                        </Badge>
                        {leader.is_verified ? <Badge className="bg-emerald-500/10 text-emerald-600 border-0">
                            <CheckCircle className="h-3 w-3 mr-1" />
                            Verificado
                          </Badge> : <Badge variant="outline" className="text-amber-600 border-amber-300 bg-amber-50">
                            Não verificado
                          </Badge>}
                        {/* Badge de Cartão Digital */}
                        {leader.passkit_pass_installed ? <Badge className="bg-violet-500/10 text-violet-600 border-0">
                            <Smartphone className="h-3 w-3 mr-1" />
                            Cartão instalado
                          </Badge> : leader.passkit_member_id ? <Badge variant="outline" className="text-gray-500 border-gray-300">
                            <Smartphone className="h-3 w-3 mr-1" />
                      ...
                      <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-muted-foreground">
                        {leader.telefone && <span className="flex items-center gap-1.5">
                            <Phone className="h-4 w-4" />
                            {formatPhone(leader.telefone)}
                          </span>}
                        {leader.email && <span className="flex items-center gap-1.5">
                  ...
                  <div className="flex flex-col gap-1 flex-shrink-0">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleCopyAffiliateLink(leader)} title="Copiar link de indicação">
                      {copiedId === leader.id ? <CheckCircle className="h-4 w-4 text-green-500" /> : <Copy className="h-4 w-4" />}
                    </Button>
      ...
      {/* Paginação */}
      {totalPages > 1 && <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
