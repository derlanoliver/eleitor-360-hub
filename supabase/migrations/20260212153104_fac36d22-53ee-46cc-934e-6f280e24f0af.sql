...
-- Add body_html column to store the final rendered email
...
BEGIN
  ...
  em AS (
    SELECT 'email' AS channel, el.subject,
           COALESCE(el.body_html, et.conteudo_html, el.subject) AS message,
           el.status, el.sent_at, NULL AS phone, el.to_email, el.error_message,
           NULL::timestamptz AS delivered_at, NULL::timestamptz AS read_at, el.created_at
  ...
  sm AS (
    SELECT 'sms' AS channel, sms.message AS subject, sms.message, sms.status, sms.sent_at,
           sms.phone, NULL AS to_email, sms.error_message, sms.delivered_at, sms.read_at, sms.created_at
