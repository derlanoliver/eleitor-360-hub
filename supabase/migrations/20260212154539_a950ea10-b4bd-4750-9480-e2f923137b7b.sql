...
BEGIN
  ...
  -- events participated (by leader_id only - fast)
  ...
  -- communications: use leader_id index for emails, phone suffix for wa/sms
  WITH em_raw AS (
    SELECT el.subject AS raw_subject,
  ...
  wa AS (
    ...
    WHERE v_phone_suffix <> '' AND wm.phone LIKE '%' || v_phone_suffix
    ORDER BY wm.created_at DESC LIMIT 20
  ),
  sm AS (
    ...
    WHERE v_phone_suffix <> '' AND sms.phone LIKE '%' || v_phone_suffix
  ...
  -- tree totals (limit depth to avoid timeout)
  WITH RECURSIVE tree AS (
    SELECT id, pontuacao_total, cadastros, 1 AS depth
    FROM public.lideres WHERE parent_leader_id = p_leader_id AND is_active = true
    UNION ALL
    SELECT l.id, l.pontuacao_total, l.cadastros, t.depth + 1
    FROM public.lideres l JOIN tree t ON l.parent_leader_id = t.id
    WHERE l.is_active = true AND t.depth < 10
  )
...
-- Add indexes to speed up phone suffix matching
CREATE INDEX IF NOT EXISTS idx_whatsapp_messages_phone ON public.whatsapp_messages (phone);
CREATE INDEX IF NOT EXISTS idx_sms_messages_phone ON public.sms_messages (phone);
CREATE INDEX IF NOT EXISTS idx_email_logs_leader_id ON public.email_logs (leader_id);
CREATE INDEX IF NOT EXISTS idx_email_logs_to_email ON public.email_logs (LOWER(to_email));
