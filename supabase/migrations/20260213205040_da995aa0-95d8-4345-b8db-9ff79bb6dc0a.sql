...
CREATE OR REPLACE FUNCTION public.process_verification_keyword(_token TEXT, _phone TEXT)
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    IF FOUND THEN
      ...
      -- Validate phone DIRECTLY against the leader's registered phone
      ...
      IF _registered_digits = '' OR _phone_digits <> _registered_digits THEN
        -- Phone mismatch - someone else is trying to verify with this code
        RETURN QUERY SELECT false, 'leader'::TEXT, _leader.id, _leader.nome_completo, 'phone_mismatch'::TEXT;
        RETURN;
      END IF;
      ...
      IF NOT FOUND THEN
        -- Phone matches but no pending verification record exists
        -- Create one on the fly so the consent flow can proceed
        ...
        -- Return success so consent flow starts
        RETURN QUERY SELECT true, 'leader'::TEXT, _leader.id, _leader.nome_completo, NULL::TEXT;
        RETURN;
      END IF;
