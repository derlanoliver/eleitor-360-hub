...
-- Drop ALL overloads of register_leader_from_affiliate
DROP FUNCTION IF EXISTS public.register_leader_from_affiliate(text,text,text,text,uuid,text,uuid);
DROP FUNCTION IF EXISTS public.register_leader_from_affiliate(text,text,text,uuid,text,date);
DROP FUNCTION IF EXISTS public.register_leader_from_affiliate(text,text,text,uuid,text);

-- Recreate with the correct signature + is_already_contact field
...
BEGIN
  ...
  -- Check if phone already exists in office_contacts (already a contact, cannot become leader via link)
  SELECT id INTO v_existing_contact_id
  FROM office_contacts
  WHERE telefone_norm = p_telefone_norm AND is_active = true
  LIMIT 1;

  IF v_existing_contact_id IS NOT NULL THEN
    RETURN QUERY SELECT
      NULL::UUID, NULL::TEXT, NULL::TEXT, FALSE, FALSE, NULL::TEXT, FALSE, TRUE;
    RETURN;
  END IF;
