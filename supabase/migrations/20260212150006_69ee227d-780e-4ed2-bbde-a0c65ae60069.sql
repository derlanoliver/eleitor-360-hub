...
BEGIN
  ...
  -- Events participated (via event_registrations matching leader phone/email)
  ...
  FROM (
    ...
    WHERE (er.whatsapp ILIKE '%' || right(regexp_replace(COALESCE(v_leader.telefone,''), '\D', '', 'g'), 8) || '%'
           OR (v_leader.email IS NOT NULL AND er.email = v_leader.email))
      AND v_leader.telefone IS NOT NULL
  ...
  FROM (
    SELECT sm.message as subject, sm.message, sm.status, sm.sent_at, sm.phone, sm.error_message, sm.delivered_at, sm.read_at, sm.created_at
